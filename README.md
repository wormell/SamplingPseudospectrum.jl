...
This works for operators estimated least squares: generalised eigenvalue problems for rank-one sample matrices can be estimated at present through `Pestimate([0],ΨX,ΨY;...)` where `ΨX`, `ΨY` are vectors containing the left and right vectors creating the matrices.
